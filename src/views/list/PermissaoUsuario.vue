        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <table class="table table-hover table-striped table-sm">
              ...
              <tbody v-else>
                <tr>
                  <td colspan="3" align="center">Nenhum usuario encontrado</td>
                </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  mounted() {
    Conexao.get("/UsuariosPermissao/" + this.id)
      .then((resposta) => {
  ...
  methods: {
    excluirPermissao(item) {
      Conexao.delete(
        "/UsuariosPermissao/" + item.idUsuario + "/" + item.idPermissao
      ).then((resposta) => {
        if (resposta.data.isOk) {
    ...
    pesquisar() {
      if (this.query != "") {
        Conexao.get(
          "/UsuariosPermissao/PesquisarUsuariosPermissao/" +
            this.id +
            "/" +
            this.query
        ).then((resposta) => {
